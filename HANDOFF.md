...
Фактически внедрено и опубликовано в ветке `test` коммитом `9af07d0`:

- `src/hydrowatch_baseline/experiments.py` формирует план только для
  не-baseline событий и добавляет контрольные пары к Score каждого fold;
- `scripts/run_experiments.py` сохраняет план, веса, предсказания,
  fold-отчёты и сводные CSV/JSON; из training history извлекаются validation
  IoU и F1;
- `metric.py` поддерживает вычисление Score и отчёта на явном наборе pair_id,
  поэтому обучающие события не попадают в метрику удержанного fold;
- добавлены unit-тесты плана и выборки fold, локально прошли 11 тестов.

Ни один fold пока не запускался: в свежем клоне отсутствуют `data/`, веса и
Sentinel GeoTIFF. CI создан и запускается при push, но его результат из этой
среды не проверялся.

Исправление после проверки автоматизации: baseline-контроли теперь исключаются
из обучающей выборки и входят в каждый validation fold вместе с удержанным
паводковым событием. Inference ограничен тем же набором pair_id, поэтому fold
submission совместим с локальным Score. `final.weights.h5` сохраняет глобально
лучший checkpoint по `val_loss`, а итоговые IoU/F1 берутся из соответствующей
лучшей эпохи. Регрессионные проверки добавлены; локально проходят 15 тестов.
